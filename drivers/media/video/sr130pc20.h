...
#define SR130PC20_BURST_WRITE_REGS(sd, A) ({ \
	int ret; \
		cam_info("BURST_WRITE_REGS: reg_name=%s from setfile\n", #A); \
		ret = sr130pc20_write_regs_from_sd(sd, #A); \
		ret; \
	})
#else

#define SR130PC20_BURST_WRITE_REGS(sd, A) \
	sr130pc20_burst_write_regs(sd, A, (sizeof(A) / sizeof(A[0])), #A)
#endif

#define SR130PC20_WRITE_LIST(A) \
	sr130pc20_i2c_write_list(A, (sizeof(A) / sizeof(A[0])), #A)
#define SR130PC20_BURST_WRITE_LIST(A) \
	sr130pc20_i2c_burst_write_list(sd, A, (sizeof(A) / sizeof(A[0])), #A)
...
#ifdef CONFIG_VIDEO_SR130PC20_P8
	struct regset_table antibanding;
#endif /* CONFIG_VIDEO_SR130PC20_P8 */
};
