...
#define CHECK_MOVI_VHX4_41				\
	(card->cid.manfid == 0x15 &&			\
	 (card->cid.prod_rev == 0x1c || card->cid.prod_rev == 0x1d) &&	\
	 card->ext_csd.feature_support & MMC_POWEROFF_NOTIFY_FEATURE)

/*
 * If moviNAND VHX 4.5 device
 * enable PON to force.
 * prod_rev is 0xFA or 0xFB.
 */
#define CHECK_MOVI_VHX4_5				\
	(card->cid.manfid == 0x15 &&			\
	 (card->cid.prod_rev == 0xFA || card->cid.prod_rev == 0xFB) &&	\
	 card->ext_csd.feature_support & MMC_POWEROFF_NOTIFY_FEATURE)
...
static int mmc_read_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	if (card->ext_csd.rev >= 5) {
		...
		/* enable PON feature if moviNAND VHX devices */
		if ((ext_csd[EXT_CSD_VENDOR_SPECIFIC_FIELD + 0] & 0x2) &&
				(card->cid.manfid == 0x15))
			card->ext_csd.feature_support |= MMC_POWEROFF_NOTIFY_FEATURE;
	...
	/* moviNAND VHX 4.41 deivce */
	if (card->ext_csd.rev == 5 && CHECK_MOVI_VHX4_41) {
	...
	struct mmc_card *oldcard)
{
	...
	if ((host->caps2 & MMC_CAP2_POWEROFF_NOTIFY) &&
	    (((card->ext_csd.rev >= 6) && CHECK_MOVI_VHX4_5) ||
	     CHECK_MOVI_VHX4_41)) {
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
