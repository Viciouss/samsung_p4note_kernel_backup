...
static int synaptics_ts_set_func(struct synaptics_drv_data *data)
{
	...
	if (synaptics_ts_set_func_info(data)) {
	...
	u32 finger_status)
{
	...
	if ((touch_key_data & 0x1) != 0)
		input_report_key(data->input, KEY_MENU, 1);
	else
		input_report_key(data->input, KEY_MENU, 0);
	input_sync(data->input);

	if ((touch_key_data & 0x2) != 0)
		input_report_key(data->input, KEY_BACK, 1);
	else	
		input_report_key(data->input, KEY_BACK, 0);
	input_sync(data->input);
