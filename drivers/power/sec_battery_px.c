			     ...
			     const char *buf, size_t count)
{
	...
	case FTA_MODE:
		if (sscanf(buf, "%d\n", &x) == 1) {
			if (x == 1)
				debug_batterydata->fta_mode = true;
			else
				debug_batterydata->fta_mode = false;
			ret = count;
			pr_info("%s : fta_mode (%d) \n", __func__,
				debug_batterydata->fta_mode);
			}
		break;
#endif
